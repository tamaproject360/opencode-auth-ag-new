...
const warmupAttemptCounts = new Map();
...
function extractVerificationErrorDetails(bodyText) {
    ...
    if (!validationRequired) {
        const hasStrongVerificationPhrase = lowerBody.includes("verify your account") ||
...
function isLikelyHardForbidden403(bodyText, message) {
    const normalized = decodeEscapedText(`${bodyText}\n${message ?? ""}`).toLowerCase();
    if (!normalized.trim()) {
        return false;
    }
...
function markStoredAccountForbidden(account, reason) {
    let changed = false;
...
function clearStoredAccountForbidden(account, enableIfRequired = false) {
...
export const createAntigravityPlugin = (providerId) => async ({ client, directory }) => {
    ...
    const eventHandler = async (input) => {
        ...
        if (input.event.type === "session.created") {
            ...
            // Show active account toast on session.created (TUI is ready at this point)
            if (config.show_active_account &&
                !config.quiet_mode &&
                !(config.toast_scope === "root_only" && isChildSession) &&
                activeAccountManager) {
                const claudeAccount = activeAccountManager.getCurrentAccountForFamily("claude");
                const geminiAccount = activeAccountManager.getCurrentAccountForFamily("gemini");
                const totalAccounts = activeAccountManager.getAccountCount();
                let accountLine = "";
                if (claudeAccount &&
                    geminiAccount &&
        ...
        auth: {
            ...
            loader: async (getAuth, provider) => {
                ...
                try {
                    ...
                    fs.writeFileSync("C:\\Users\\harri\\.config\\opencode\\antigravity-debug.json", JSON.stringify(debugInfo, null, 2));
                }
                catch (e) {
                    // ignore
                }
                                ...
                                for (let i = 0; i < ANTIGRAVITY_ENDPOINT_FALLBACKS.length; i++) {
                                    ...
                                    try {
                                        ...
                                        if (prepared.contextOverflowResponse) {
                                            ...
                                            return createSyntheticErrorResponse("✅ Context was too long — your context has been automatically compacted.\n\nPlease **resend your last message** to continue.", prepared.requestedModel || prepared.effectiveModel || "unknown");
                                        }
                                        ...
                                        if (response.status === 403) {
                                            ...
                                            if (isLikelyHardForbidden403(errorBodyText, extracted.message)) {
                                                const forbiddenReason = extracted.message ??
                                                    "Access forbidden (403). Account may be restricted or banned.";
                                                const cooldownMs = 30 * 60 * 1000;
                                                accountManager.markAccountForbidden(account.index, forbiddenReason);
                                                accountManager.markAccountCoolingDown(account, cooldownMs, "auth-failure");
                                                const label = account.email || `Account ${account.index + 1}`;
                                                if (accountManager.shouldShowAccountToast(account.index, 60000)) {
                                                    await showToast(`⛔ ${label} got a hard 403 auth block and was disabled.`, "warning");
                                                    accountManager.markToastShown(account.index);
                                                }
                                                pushDebug(`forbidden-403: disabled account ${account.index}`);
                                                getHealthTracker().recordFailure(account.index);
                                                lastFailure = {
                                                    response,
                                                    streaming: prepared.streaming,
                                            ...
                                            pushDebug(`forbidden-403: account ${account.index} not auto-disabled (no hard-forbidden signal)`);
                                        }
                        ...
                        if (inputs) {
                                ...
                                while (true) {
                                        ...
                                        for (const res of results) {
                                            ...
                                            if (res.quota?.groups) {
                                                ...
                                                if (acc) {
                                                    ...
                                                    if (claudeQuota) {
                                                        if (typeof claudeQuota.remainingFraction ===
                                                            "number" &&
                                                            claudeQuota.remainingFraction <= 0 &&
                                                            claudeQuota.resetTime) {
                                                            acc.rateLimitResetTimes.claude = Date.parse(claudeQuota.resetTime);
                                                        }
                                                        else {
                                                    ...
                                                    const geminiExhausted = geminiPro &&
                                                        typeof geminiPro.remainingFraction === "number" &&
                                                        geminiPro.remainingFraction <= 0;
                                                    const flashExhausted = geminiFlash &&
                                                        typeof geminiFlash.remainingFraction ===
                                    ...
                                    if (menuResult.mode === "manage") {
                                        ...
                                        if (menuResult.selectAccountIndex !== undefined) {
                                            const acc = existingStorage.accounts[menuResult.selectAccountIndex];
                                            if (acc) {
                                                if (acc.enabled === false) {
                                                    ...
                                                    await saveAccounts(existingStorage);
                                                    activeAccountManager?.setCurrentAccount(menuResult.selectAccountIndex);
                                    ...
                                    if (menuResult.mode === "reset-all-status") {
                                        ...
                                        for (let i = 0; i < existingStorage.accounts.length; i++) {
                                            ...
                                            if (account.verificationRequired) {
                                                ...
                                                account.verificationRequiredReason = undefined;
                                                account.verificationUrl = undefined;
                                                activeAccountManager?.clearAccountVerificationRequired(i, true);
                                            }
                            ...
                            while (accounts.length < MAX_OAUTH_ACCOUNTS) {
                                ...
                                if (result.type === "failed") {
                                    ...
                                    console.warn(`[opencode-auth-ag-new] Skipping failed account ${accounts.length + 1}: ${result.error}`);
                                    break;
                                }
                                ...
                                try {
                                    if (refreshAccountIndex !== undefined) {
                                        ...
                                        if (currentStorage) {
                                            ...
                                            const parts = parseRefreshParts(result.refresh);
                                            if (parts.refreshToken) {
                                                const existingAccount = updatedAccounts[refreshAccountIndex];
